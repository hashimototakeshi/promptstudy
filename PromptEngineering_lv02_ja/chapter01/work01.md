...
これまで皆さんは、ChatGPTやGemini、Claudeなどのブラウザを開いてAIに質問を送ってきました。
これから学習するDifyでは、利用したいモデルのAPIキーを設定することで生成AIを利用できるようになります。

6章以降では、無料で利用できるGeminiのAPIキー※を活用しますので、まずはGoogle AI StudioからAPIキーを取得しましょう。

※リクエスト数の上限は、[レート制限](https://ai.google.dev/gemini-api/docs/rate-limits?hl=ja) から確認できます。
